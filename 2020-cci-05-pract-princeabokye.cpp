...
/*      REPORT ON FIRST PROGRAM
All that the first program is doing is finding the sum of the first n numbers using a function

	REPORT ON SECOND PROGRAM
This program create a list with size n and allows you to either insert items into a list, delete items from a list, find items in the list, test if the list is empty, test if the list is full, displays the content of the list or fill the list with random numbers.

	REPORT ON THIRD PROGRAM
This program create a stack with size n and allows you to either insert items into a stack, delete items from a stack, find items in the stack, test if the stack is empty, test if the stack is full, displays the content of the stack or fill the stack with random numbers.

	REPORT ON FOURTH PROGRAM
This program create a queue with size n and allows you to either insert items into a queue, delete items from a queue, find items in the queue, test if the queue is empty, test if the queue is full, displays the content of the queue or fill the queue with random numbers.

	REPORT ON FIFTH PROGRAM
This program takes in random numbers and perform the selection sort operation on them.

	REPORT ON SIXTH PROGRAM
This program takes in random numbers and perform the merge sort operation on them.

	REPORT ON SEVENTH PROGRAM
Here in this program we have taken the array as an input from the user along with the key to be searched. We have created a separate function for Linear Search. When the function is called we have to run a loop n times where n is the number of elements in an array. In each iteration we are comparing the value of key with elements of array in increasing order of array index. If key is equal to one of the array elements we print the value of index at which we found them to be equal..

	REPORT ON EIGHTH PROGRAM
Here in this program we have first requested the array of numbers and the value to be searched from the user. Since binary search is applied upon a sorted array we have created a separate bubble sort function to sort the array before searching. After sorting the array we have passed the array to the function called BinarySearch(array, keynum, num) having array, key and the size of array as its parameters. In BinarySearch(array, keynum, num) we first find the middle index of the array, and then compare the key with the middle element of array i.e. array[middle]. If key is less than the middle element of array we divide the array in to two halves i.e. low to mid-1 and (mid+1) to high, where mid is the middle index, low is the starting and high is the ending index of the array. After dividing the array we start searching in the left half of the array i.e. low to mid-1. If the key is greater than the middle element then we search in the right half of the array i.e mid+1 to high. In each iteration we divide the array until it has only 1 element left. If we are able to find the element we print “SEARCH SUCCESSFUL” otherwise we print “SEARCH FAILED”.

	REPORT ON NINTH PROGRAM
This program uses recursion, performs traversal operation across the nodes in a tree. The tree I used is the binary search tree. The user generates a tree by inserting integers. The user is also asked to select one of the three modes of traversal, that is, infix(inorder), prefix(preorder) and postfix(postorder). The user can use any of the three options to see the result. A binary search tree follows a concept of the nodes whose numbers are lesser than the parent (pointed node are linked to the left) and the nodes whose number are greater than the parent (pointed node are linked to the right).
*/
